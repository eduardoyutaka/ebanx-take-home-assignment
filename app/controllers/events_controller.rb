class EventsController < ApplicationController
  def create
    ...
    when 'deposit'
      ...
      render json: { destination: { id: @account.account_id, balance: @account.balance } }, status: :created
    when 'withdraw'
      @account = Account.find_by(account_id: params[:origin])
      if @account
        @account.update(balance: @account.balance - params[:amount])
        render json: { origin: { id: @account.account_id, balance: @account.balance } }, status: :created
      else
    ...
    when 'transfer'
      ...
      if @origin_account
        ...
        render json: { origin: { id: @origin_account.id, balance: @origin_account.balance }, destination: { id: @destination_account.id, balance: @destination_account.balance } }, status: :created
      else
        retrn json: 0, status: :not_found
      end
    end
  end
